...
## Running a Task
1. Ask the user (via `AskUserQuestion`) which model to run (`gpt-5.1-codex` or `gpt-5.1-codex-mini
` or `gpt-5-codex-mini` or `gpt-5.1`) AND which reasoning effort to use (`high`, `medium`, or `low`) in a **single prompt with two questions**.
