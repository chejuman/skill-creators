#!/usr/bin/env python3
"""
Search Docs for Wireframe Design Studio
Full-text search across all .wireframe-docs/ documentation.

Usage:
    python3 search_docs.py --query "sidebar"
    python3 search_docs.py --query "Card" --type specs
"""
...
def search_docs(query: str, base_path: str = ".", doc_type: Optional[str] = None,
    ...
    if not docs.exists():
        return {"error": ".wireframe-docs directory not found", "results": []}

    if doc_type:
        search_dirs = [docs / doc_type]
    else:
        search_dirs = [
            docs / "discovery",
            docs / "designs",
...
"""
    ...
    for file_path, results in by_file.items():
        rel_path = file_path.replace(WIREFRAME_DOCS_DIR + "/", "")
...
def main():
    parser = argparse.ArgumentParser(description="Search wireframe documentation")
    parser.add_argument("--query", required=True, help="Search query")
    parser.add_argument("--path", default=".", help="Base path")
    parser.add_argument("--type", choices=["discovery", "designs", "specs", "tasks", "tracking"],
